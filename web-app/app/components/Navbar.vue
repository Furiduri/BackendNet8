...
<template>
    ...
    <div class="container mx-auto px-4">
      <div class="flex items-center justify-between py-4">
        <div>
          <img class="max-h-12" src="/vite.png" alt="Logo">
        </div>

        <ul class="hidden sm:flex sm:items-center">
          <li v-for="item in items" :key="item.title">
            <NuxtLink :to="item.to" class="flex items-center justify-between text-gray-800 text-sm font-semibold hover:text-purple-600 mr-4">
              <Icono :NameIcon="item.icon" />
              {{ $t(item.title) }}</NuxtLink>
          </li>
        </ul>

        <div class="hidden sm:flex sm:items-center">          
          <NuxtLink to="/auth/login" class="flex items-center justify-between text-gray-800 text-sm font-semibold hover:text-purple-600 mr-4">
            <Icono NameIcon="i-mdi-login" />
            {{ $t('auth.login') }}</NuxtLink>
          <NuxtLink to="/auth/register" class="flex items-center justify-between text-gray-800 text-sm font-semibold border px-4 py-2 rounded-lg hover:text-purple-600 hover:border-purple-600">
            <Icono NameIcon="i-mdi-account-plus" />
